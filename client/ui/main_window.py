...
class MainWindow(QMainWindow):
    ...
    def dispatch_packet(self, d):
        ...
        if t == "system":
            ...
            # Xác định ID người gửi để biết dùng 'is_me' hay không
            ...
            f_name = d.get('fullname')
            avt = d.get('avatar')

            # Update chat-area avatars keyed by username (so existing bubbles update)
            if u_name:
                self.chat_area.update_avatar_cache(u_name, avt)

            # If the update is for myself, update self profile UI; otherwise notify
            if u_name == self.username:
                self.on_profile_updated_local(f_name, avt)
            else:
                self.show_toast(f"Hồ sơ của {f_name} đã cập nhật!")

        # Xử lý tin nhắn realtime (Text/File/Image)
