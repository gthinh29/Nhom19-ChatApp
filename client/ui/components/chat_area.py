...
class ChatArea(QFrame):
    ...
    def update_avatar_cache(self, username, b64):
        # Lưu vào cache và cập nhật ngay các MessageBubble hiện có
        self.avatar_cache[username] = b64
        try:
            for i in range(self.mlay.count()):
                item = self.mlay.itemAt(i)
                if not item: continue
                w = item.widget()
                if not w: continue
                # MessageBubble cung cấp update_userdata(username, avatar_b64)
                try:
                    if hasattr(w, 'update_userdata'):
                        # So sánh với sender id hoặc display name
                        sender_id = getattr(w, 'data', {}).get('sender_id')
                        sender_name = getattr(w, 'sender_name', None)
                        if sender_id == username or sender_name == username or getattr(w, 'data', {}).get('sender') == username:
                            w.update_userdata(username, b64)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def add_upload_bubble(self, filename, total_bytes, sender_name, is_me):
        # Đảm bảo không trùng bong bóng resume
    ...
    def _enrich_user_data(self, data):
        """Thêm dữ liệu phụ vào message trước khi tạo bong bóng: avatar từ cache."""
        try:
            # Prefer explicit sender_id if present
            sender_id = data.get('sender_id')
            sender = data.get('sender')

            if sender_id and sender_id in self.avatar_cache:
                data['avatar_data'] = self.avatar_cache.get(sender_id)
            elif sender and sender in self.avatar_cache:
                data['avatar_data'] = self.avatar_cache.get(sender)
            else:
                # Fallback: if message already contains avatar_data, keep it
                data.setdefault('avatar_data', data.get('avatar_data', ''))

            # Kiểm tra tên người gửi để cập nhật bubble
            # MessageBubble uses data.get('sender') as display name; leave as-is
        except Exception:
            pass
