"""Kết nối mạng và luồng nhận tin (NetworkClient, ReceiverThread)."""
...
try:
    ...
    # Đường dẫn dự phòng import Protocol
...
class ReceiverThread(QThread):
    ...
    def _process_packet(self, pkt_bytes):
        try:
            ...
            # In ra gói tin Token Resume để debug
    ...
    def _parse_raw_message(self, raw_msg):
        ...
        # Real-time profile update broadcast from server: profile_update|username|fullname|avatar_b64
        if raw_msg.startswith("profile_update|"):
            parts = raw_msg.split("|", 3)
            # parts[0] = 'profile_update'
            username = parts[1] if len(parts) > 1 else ""
            fullname = parts[2] if len(parts) > 2 else ""
            avatar = parts[3] if len(parts) > 3 else ""
            return {"type": "profile_update", "username": username, "fullname": fullname, "avatar": avatar}
